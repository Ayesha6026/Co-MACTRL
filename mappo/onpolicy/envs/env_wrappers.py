...
class ShareVecEnvMPlayer(ABC):
    ...
    def __init__(self, num_envs, observation_space, share_observation_space, action_space, enemy_action_space):
...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        cmd, data = remote.recv()
        if cmd == 'step':
            ob, enemy_obs, s_ob, reward, done, info, available_actions, available_enemy_actions = env.step(data)
            if 'bool' in done.__class__.__name__:
                if done:
            ...
            remote.send((ob, enemy_obs, s_ob, reward, done, info, available_actions, available_enemy_actions))
        elif cmd == 'reset':
            ob, s_ob, available_actions = env.reset()
...
class ShareSubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        obs, enemy_obs, share_obs, rews, dones, infos, available_actions, available_enemy_actions = zip(*results)
        return np.stack(obs), np.stack(enemy_obs), np.stack(share_obs), np.stack(rews), np.stack(dones), infos, np.stack(available_actions), np.stack(available_enemy_actions)
...
class ShareDummyVecEnvMPlayer(ShareVecEnvMPlayer):
    def __init__(self, env_fns):
        self.envs = [fn() for fn in env_fns]
        env = self.envs[0]
        ShareVecEnvMPlayer.__init__(self, len(
            env_fns), env.observation_space, env.share_observation_space, env.action_space, env.enemy_action_space)
    ...
    def step_wait(self):
        ...
        obs, enemy_obs, share_obs, rews, dones, infos, available_actions, available_enemy_actions = map(
        ...
        return obs, enemy_obs, share_obs, rews, dones, infos, available_actions, available_enemy_actions
